...
exports.getDayISO = getDayISO;
exports.computeWeeklyGrowthScore = computeWeeklyGrowthScore;
exports.computeMasteryScore = computeMasteryScore;
...
function getDayISO(date = new Date()) {
    return new Date(Date.UTC(date.getUTCFullYear(), date.getUTCMonth(), date.getUTCDate())).toISOString().slice(0, 10);
}
function computeWeeklyGrowthScore(s) {
...
function computeMasteryScore(s) {
    const attempted = s.questionsAttempted;
    const accuracy = attempted > 0 ? s.questionsCorrect / attempted : 0;
...
async function getWeeklyLeaderboard(weekStart, type, limit = 50) {
    const stats = await UserWeeklyStats_1.UserWeeklyStats.find({ weekStart }).lean();
    const ranked = stats
        .map((s) => {
        const score = type === "growth"
